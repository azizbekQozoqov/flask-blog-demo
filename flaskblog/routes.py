...
def login():    
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            flash(f'Account logged in as {user.username}', 'success')
            next_page = request.args.get('next')
            
            return redirect(next_page) if next_page else redirect(url_for('home'))
        else:
            flash(f'Login faild. Check your Email and password!', 'danger')
...
def account():
    ...
    if form.validate_on_submit():
        if form.picture.data:
            ...
            if current_user.image_file != 'default.jpg':
                os.remove(os.path.normpath(app.root_path + '\\' + 'static/profile_pics' + '\\' + current_user.image_file))
    ...
    return render_template('account.html', title='Account', user=user, image_file=image_file, form =form)
...
def send_reset_email(user):
    token = User.get_reset_token(expires_sec=3000)
    msg = Message('Password reset request', 'developersevenuser@gmail.com', recipients=[user.email])
    
    msg.body = f'''To reset your password, visit the following link
    {url_for('reset_token', token=token, _external=True)}
    
    AzizbekDeveloperUser - No reply
    '''
